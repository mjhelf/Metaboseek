...
MainTableModule <- function(input,
  ...
  callModule(SaveTableModule, "savetable",
             reactives = reactive({list(df = NULL,
                                        filename = file.path("Saved Tables", 
                                                             paste0(gsub("\\.csv$","",values$featureTables$tables[[values$featureTables$active]]$tablename),
  ...
  output$maintable <- renderRHandsontable({
    if(length(internalValues$inpage > 0 )){
      ...
      rhandsontable(values$featureTables$tables[[values$featureTables$active]]$df[internalValues$inpage,values$featureTables$selectedCols],
