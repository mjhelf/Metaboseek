...
#' @param reactives a list, wrapped in a \code{\link[shiny]{reactive}()} expression with settings for this module. See \code{details}.
...
#' @title MseekModules
#'
#' @description Regular modules in Metaboseek take a \code{shiny::reactiveValues} object called \code{values} as their first argument.
#'  \code{values} is the primary interface allowing communication between modules and is generated by \code{\link{MseekMinimalServer}()}.
#'   Modules expect this input to work properly, and will only work in shiny apps that provide the proper environment set up by \code{\link{MseekMinimalServer}()}.
#'   
#' @return If the server module for this Module returns something, it is described in \code{Details}.
#'   
#' @param input,output,session arguments necessary for use with \code{\link[shiny]{callModule}()}
