...
TableModule2 <- function(input,output, session,
  ...
  observeEvent(values$df,{
    ...
    internalValues$selected_cols <- NULL
      ...
      if(length(values$df) == 0 ){
         internalValues$showTable <- NULL
        }
    
    if(!is.null(values$df)){
            internalValues$row_order <- seq(nrow(values$df))
    }
      internalValues$df <- values$df
  })
                 ...
                 internalValues$sortCheck),{
                   if(!is.null(internalValues$df) && length(internalValues$df) > 0){
                     ...
                     if(internalValues$sortCheck && length(internalValues$sortBy) > 0){
                       ...
                       internalValues$row_order <- if(!is.null(values$rowFilters)){
                         seq(nrow(internalValues$df[values$rowFilters,]))}else{
                     ...
                     if(!is.null(values$colFilters)){
                     internalValues$showTable <- internalValues$df[internalValues$inpage,values$colFilters]}
                     else{
                       internalValues$showTable <- internalValues$df[internalValues$inpage,]
                     }
