...
#' @param data an MSnExp or OnDiskMSnExp object. If NULL, will assume that values$MSData$MSnExp is defined in parent environment
#' @param type specify what the scan number represents, either "acquisition", "ms1" or "ms2".
#'
#'
#' @export
getScanInfo <- function(file, number, data = NULL, type = c("acquisition", "ms1", "ms2")){
  
  if(is.null(data)){data <- values$MSData$MSnExp}
