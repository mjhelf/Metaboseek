...
EICplot <- function(bin1_p, rawcoll, binvar1 = c("all"), alli = NULL, ppr = 10, rtw=90, 
  ...
  if(!is.null(alli)){
    ...
    if (is.null(alli$rt) & (is.null(rtmin) | is.null(rtmax)) ){rtr <- data.frame(0,max(sapply(sapply(unlist(bin1_p),slot,"scantime"),max)))}
    else
    {if (!is.null(rtmin) & !is.null(rtmax)){rtr <- data.frame(rtmin,rtmax)}
        else
